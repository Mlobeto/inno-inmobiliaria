  ...
  try {
    ...
    if (search) {
      where.OR = [
        ...
        { admins: { some: { username: { contains: search, mode: 'insensitive' }, deletedAt: null } } },
      ];
    }
...
exports.createManualTenant = async (req, res) => {
  try {
    ...
    const resolvedAdminUsername = (adminUsername?.trim() || `${subdomain}_admin`).toLowerCase();
    const resolvedAdminPassword = adminPassword?.trim() || crypto.randomBytes(9).toString('base64url');

    const existingUsername = await prisma.admins.findFirst({
      where: { username: resolvedAdminUsername },
    });
    if (existingUsername) {
      return res.status(400).json({
        success: false,
        message: `El username "${resolvedAdminUsername}" ya está en uso. Elegí otro.`,
      });
    }

    temporaryPassword = adminPassword?.trim() ? adminPassword : resolvedAdminPassword;
    ...
    res.status(201).json({
      ...
      data: {
        ...
        admin: {
          ...
          temporaryPassword,
          autoGenerated: !adminPassword?.trim(),
        },
...
exports.impersonateTenant = async (req, res) => {
  try {
    ...
    if (tenant.admins.length === 0) {
      const fallbackAdmin = await prisma.admins.findFirst({
