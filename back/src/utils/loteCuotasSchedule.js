/**
 * Utilidades de cuotas de loteos (copia para el backend en Docker).
 * Fuente canónica: shared/src/utils/loteCuotasSchedule.js
 */
...
function addMonthsWithDay(baseDate, monthsToAdd, dayOfMonth = 10) {
...
function calcFinancingTotals(precioTotal, anticipo = 0, interes = 0, cantidadCuotas = 1) {
...
function formatCuotaLabel(numeroCuota) {
...
function buildCustomCuotas({ cuotasCustom, montoConInteres, anticipo = 0, fechaBase }) {
...
function previewCuotasSchedule({
  modoPlan = 'periodico',
  fechaVenta,
  precioTotal,
  anticipo = 0,
